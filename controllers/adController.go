...
func CreateRoomAd() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		count_ads, err := roomAdCollection.CountDocuments(ctx, bson.M{"user_id": request.User_id})
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
		...
		if count_ads > 0 {
			c.JSON(http.StatusBadRequest, gin.H{"error": "User can only have one room ad"})
			return
		}
...
func SearchBunkieAdDefault() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		cursor, err := bunkieAdCollection.Find(ctx, bson.M{}, opts)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
